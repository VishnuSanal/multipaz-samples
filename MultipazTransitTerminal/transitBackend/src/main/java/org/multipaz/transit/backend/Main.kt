...
/**
 * Merchant backend for MultipazTransitTerminal.
 *
 * Run with: `./gradlew :transitBackend:run`.
 *
...
fun Application.configureRouting(serverEnvironment: Deferred<ServerEnvironment>) {
    val httpHandler = CoroutineScope(Dispatchers.Default).async {
        ...
        TerminalJourneyService.register(dispatcherBuilder)
    ...
    routing {
        get("/") { call.respondText("MultipazTransitTerminal backend is running") }
        rpc("/rpc", httpHandler)
    }
}
